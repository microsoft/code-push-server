...
export class Authentication {
  ...
  public async authenticate(req: Request, res: Response, next: (err?: Error) => void) {
    ...
    try {
      ...
      const user = await this._storageInstance.getAccountByEmail(userEmail);
  ...
  public getRouter(): Router {
    ...
    router.get("/authenticated", rateLimit({ windowMs: 15 * 60 * 1000, max: 100 }), this.authenticate.bind(this), (req: Request, res: Response) => {
        req.user = {
            id: "id_0",
            email: "localdev@example.com",
            name: "Local Developer"
